...
export const authService = {
  ...
  hasRole(role: string): boolean {
    ...
    try {
      ...
      // Handle case where groups might be a single string or an array
  ...
  isAuthenticated(): boolean {
    ...
    // Check if token is expired - don't consider expired tokens as authenticated
    const expired = isTokenExpired(token);
    console.log("Token expiration check:", { expired });

    if (expired) {
      console.log("Token is expired - not authenticated");
      return false;
    }
